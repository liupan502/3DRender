...
layout(location = 7) in vec3 current_pdc_pos;
layout(location = 8) in vec3 pre_pdc_pos;
layout(location = 0) out vec4 uFragColor;
layout(location = 1) out vec4 velocity_color;
...
void main() {
   #ifdef VERTEX_COLOR
   uFragColor = vertex_color;
   #else
   uFragColor = texture(tex_sampler, frag_tex_coord);
   #endif
   velocity_color = vec4(0.0f, 0.0, 0.0, 1.0);
   velocity_color.xy = (pre_pdc_pos - current_pdc_pos).xy;
   if (frag_uni_buf.jitter_info.x < 0.0001) {
      velocity_color.xy = vec2(1000.0, 1000.0);
   }
   ...
   // uFragColor = vec4(1.0, 0.0, 0.0, 1.0f); 
}