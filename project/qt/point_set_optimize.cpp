...
void choose_key_point(const std::vector<float>& input, int start_idx,
    ...
    float max_offset = force_find ? 0.0f : delta;
    key_point_idx = -1;
...
void point_set_simplification(const std::vector<float>& input, std::vector<float>& output, std::vector<uint16_t>& output_idx) {
    ...
    if (key_point_idx0 > 0) {
        ...
        choose_key_point(input, key_point_idx0, point_num - 1, false, delta, key_point_idx2);
    }

    if (key_point_idx1 > 0) {
        choose_key_point(input, 0, key_point_idx1, delta, false, key_point_idx3);
        choose_key_point(input, key_point_idx1, key_point_idx0, false, delta, key_point_idx4);
        if (key_point_idx3 > 0) {
    ...
    if (key_point_idx2 > 0) {
        ...
        choose_key_point(input, key_point_idx2, point_num - 1, false, delta, key_point_idx6);
        if (key_point_idx5 > 0) {
